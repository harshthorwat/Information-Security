...
char RemoveFun(char key1[],char key[]);      //Removes Repeted alphabets.
char RemoveJ(char alp[]);                    //Plases 'I' in the place of 'J'.
char Caps(char key1[]);                      //Converts string in uppercase.
...
int main()
{
	...
	RemoveJ(key1);
	RemoveFun(key1,key);
	for(i=0;i<26;i++)            //Create array of alphabets from A to Z.
	{
		alp[i]=ch;
		ch++;
	}
